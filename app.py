...
║   Built by Mohamed Ahshaan  ·  v3.0 Final           ║
╚══════════════════════════════════════════════════════╝
"""
...
# ═══════════════════════════════════════════════════════
st.set_page_config(
    page_title="DataSci Studio v3.0 · Ahshaan",
    page_icon="⚡",
    layout="wide",
...
def build_clf_models(use_cw):
    ...
    m = {
        ...
        "Random Forest":        RandomForestClassifier(n_estimators=120, random_state=42, class_weight=cw),
    ...
    if XGBOOST_OK:
        m["XGBoost"] = XGBClassifier(use_label_encoder=False, eval_metric='logloss',
...
def build_reg_models():
    ...
    if XGBOOST_OK:
        m["XGBoost"] = XGBRegressor(random_state=42, verbosity=0)
    return m
...
def p_shap(model, X_tr, feats, name):
    ...
    try:
        if hasattr(model, "predict_proba"):
            exp = shap.TreeExplainer(model) if hasattr(model, "feature_importances_") else shap.KernelExplainer(model.predict_proba, shap.sample(X_tr, 50))
        else:
            exp = shap.TreeExplainer(model) if hasattr(model, "feature_importances_") else shap.KernelExplainer(model.predict, shap.sample(X_tr, 50))
        sv = exp.shap_values(X_tr[:100] if len(X_tr) > 100 else X_tr)
        if isinstance(sv, list): sv = sv[1]
        pstyle(); fig, ax = plt.subplots(figsize=(7, max(4, len(feats)*.35)))
        shap.summary_plot(sv, X_tr[:100] if len(X_tr) > 100 else X_tr,
                          feature_names=feats, show=False, plot_type="bar")
...
with st.sidebar:
    ...
    with c1:
        ...
        <p style='font-size:.66rem;color:{T3};font-family:"Fira Code",monospace;margin:0;letter-spacing:2px;'>BY AHSHAAN · v2.0</p>
        """, unsafe_allow_html=True)
    with c2:
...
def page_app():
    ...
    with c1:
        if miss_c.empty: sb("✅ No missing values detected.")
            ...
            }).sort_values("Missing %", ascending=False), use_container_width=True, hide_index=True)
    with c2:
